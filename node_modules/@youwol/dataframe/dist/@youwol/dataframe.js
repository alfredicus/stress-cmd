 ...
 * @category Algorithms
 */
const reduce = (series, callback) => {
    if (_serie__WEBPACK_IMPORTED_MODULE_0__["Serie"].isSerie(series)) {
        const S = series;
        const count = S.count;
    ...
    series.forEach(s => {
        if (s.count !== count)
            throw new Error('All series must have the same nbr of elements (count)');
    });
    let R = undefined;
    const args = new Array(n); //.fill([])
    let isArray = true;
    let id = 0;
    for (let i = 0; i < count; ++i) {
        for (let j = 0; j < n; ++j) {
 ...
 */
class Manager {
    constructor(df, options = undefined) {
        this.df = df;
        this.ds_ = [];
        if (options)
            this.ds_ = options;
 ...
 */
class Serie {
    constructor(array, itemSize, shared, userData = {}) {
        /**
         *
         * Mutable dictionary to store consumer data (context information of the usage)
         */
     ...
     */
    static create({ array, itemSize, userData }) {
 ...
 * @category Creation
 */
function createEmptySerie({ Type, count, itemSize = 1, shared = false, userData }) {
    if (itemSize <= 0)
        throw new Error('itemSize must be > 0');
    if (count <= 0)
    ...
    return _serie__WEBPACK_IMPORTED_MODULE_0__["Serie"].create({ array: new Type(new ArrayBuffer(length)), itemSize, userData });
}
